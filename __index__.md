...
> * <span id='文件_key'>key.yml</span>     >>>>>     密钥索引
> * backup.md     >>>>>     备份说明
...
一级结构根据**[价值-状态体系](#说明_价值-状态体系)**分为**五大区**。
...
本区域以**前缀**分两类二级结构[示例](#示例_Casual二级结构)
...
全域的详细备份方式以及备份计划记录在[backup.md](#实例)中(无格式要求)，定期备份时间(MUST)需要在[Index](#Index)生成触发器。
...
密钥索引-唯一对应的密钥-说明 会成组储存[key.yml](#文件)中。(用[常用密钥](#Access)加密。)
...
* 密钥索引为"0"，(MUST)代表无密码
* 密钥(OPTIONAL)可以自定义，或者用[Access](#Access)提供的方法生成。
...
### <span id='示例_Casual二级结构'>Casual二级结构示例</span>
